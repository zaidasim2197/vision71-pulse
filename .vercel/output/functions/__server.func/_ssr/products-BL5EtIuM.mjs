//#region node_modules/.nitro/vite/services/ssr/assets/products-BL5EtIuM.js
var columns = [
	"product_id",
	"sku",
	"product_name",
	"category",
	"unit_price",
	"unit_cost",
	"reorder_level",
	"preferred_stock_level",
	"product_status"
];
var rows = [
	[
		"PRD-0001",
		"F-0001",
		"Storage Credenza",
		"Furniture",
		49e3,
		35150,
		9,
		46,
		"Active"
	],
	[
		"PRD-0002",
		"E-0002",
		"Voltage Stabilizer 5KVA",
		"Electrical",
		52050,
		38100,
		33,
		139,
		"Active"
	],
	[
		"PRD-0003",
		"IS-0003",
		"Portable Air Compressor",
		"Industrial Supplies",
		88300,
		65e3,
		10,
		21,
		"Active"
	],
	[
		"PRD-0004",
		"E-0004",
		"Emergency Light",
		"Electrical",
		7100,
		4600,
		13,
		57,
		"Active"
	],
	[
		"PRD-0005",
		"SE-0005",
		"Fire Extinguisher 6kg",
		"Safety Equipment",
		11500,
		7900,
		15,
		110,
		"Active"
	],
	[
		"PRD-0006",
		"IS-0006",
		"Angle Grinder",
		"Industrial Supplies",
		27e3,
		19650,
		8,
		50,
		"Active"
	],
	[
		"PRD-0007",
		"IN-0007",
		"Cat6 Cable Box 305m",
		"IT & Networking",
		33250,
		23700,
		10,
		53,
		"Active"
	],
	[
		"PRD-0008",
		"IN-0008",
		"Fiber Media Converter",
		"IT & Networking",
		11400,
		7800,
		4,
		51,
		"Active"
	],
	[
		"PRD-0009",
		"OE-0009",
		"Laser Printer",
		"Office Equipment",
		75050,
		56e3,
		12,
		52,
		"Active"
	],
	[
		"PRD-0010",
		"IN-0010",
		"24-Port Managed Network Switch",
		"IT & Networking",
		50450,
		37650,
		15,
		56,
		"Active"
	],
	[
		"PRD-0011",
		"F-0011",
		"Executive Office Chair",
		"Furniture",
		50250,
		35950,
		10,
		58,
		"Active"
	],
	[
		"PRD-0012",
		"IS-0012",
		"Industrial Tool Kit",
		"Industrial Supplies",
		42850,
		31750,
		14,
		43,
		"Active"
	],
	[
		"PRD-0013",
		"C-0013",
		"Packaging Tape Carton",
		"Consumables",
		8750,
		5350,
		29,
		155,
		"Active"
	],
	[
		"PRD-0014",
		"F-0014",
		"Conference Table 10 Person",
		"Furniture",
		199750,
		146450,
		14,
		37,
		"Active"
	],
	[
		"PRD-0015",
		"OE-0015",
		"Barcode Scanner",
		"Office Equipment",
		19300,
		12700,
		11,
		30,
		"Active"
	],
	[
		"PRD-0016",
		"IS-0016",
		"Cordless Screwdriver Set",
		"Industrial Supplies",
		18750,
		12900,
		11,
		55,
		"Active"
	],
	[
		"PRD-0017",
		"OE-0017",
		"Cash Drawer",
		"Office Equipment",
		15250,
		1e4,
		9,
		37,
		"Active"
	],
	[
		"PRD-0018",
		"IS-0018",
		"Heavy Duty Drill Machine",
		"Industrial Supplies",
		39400,
		29550,
		11,
		56,
		"Active"
	],
	[
		"PRD-0019",
		"E-0019",
		"Circuit Breaker 32A",
		"Electrical",
		2950,
		1850,
		33,
		54,
		"Active"
	],
	[
		"PRD-0020",
		"F-0020",
		"Ergonomic Task Chair",
		"Furniture",
		31900,
		22100,
		13,
		31,
		"Active"
	],
	[
		"PRD-0021",
		"F-0021",
		"Executive Desk",
		"Furniture",
		151550,
		109750,
		6,
		48,
		"Active"
	],
	[
		"PRD-0022",
		"IS-0022",
		"Digital Multimeter",
		"Industrial Supplies",
		13950,
		9250,
		9,
		51,
		"Active"
	],
	[
		"PRD-0023",
		"OE-0023",
		"Conference Speakerphone",
		"Office Equipment",
		46600,
		33950,
		8,
		44,
		"Active"
	],
	[
		"PRD-0024",
		"C-0024",
		"Printer Toner Cartridge",
		"Consumables",
		17950,
		12350,
		46,
		146,
		"Active"
	],
	[
		"PRD-0025",
		"F-0025",
		"Steel Filing Cabinet",
		"Furniture",
		32700,
		22850,
		5,
		60,
		"Active"
	],
	[
		"PRD-0026",
		"C-0026",
		"Printer Label Roll Pack",
		"Consumables",
		5050,
		3050,
		38,
		202,
		"Active"
	],
	[
		"PRD-0027",
		"C-0027",
		"Disposable Gloves Carton",
		"Consumables",
		11500,
		7200,
		36,
		158,
		"Active"
	],
	[
		"PRD-0028",
		"C-0028",
		"Cleaning Tissue Carton",
		"Consumables",
		6400,
		4200,
		60,
		140,
		"Active"
	],
	[
		"PRD-0029",
		"IS-0029",
		"Industrial Ladder",
		"Industrial Supplies",
		30900,
		21650,
		6,
		42,
		"Active"
	],
	[
		"PRD-0030",
		"E-0030",
		"Automatic Voltage Regulator",
		"Electrical",
		35850,
		25550,
		17,
		56,
		"Active"
	],
	[
		"PRD-0031",
		"SE-0031",
		"Reflective Safety Vest",
		"Safety Equipment",
		1600,
		900,
		24,
		109,
		"Active"
	],
	[
		"PRD-0032",
		"OE-0032",
		"Interactive Display 65 Inch",
		"Office Equipment",
		383350,
		305700,
		4,
		39,
		"Active"
	],
	[
		"PRD-0033",
		"IN-0033",
		"48-Port Managed Network Switch",
		"IT & Networking",
		88100,
		66350,
		5,
		21,
		"Active"
	],
	[
		"PRD-0034",
		"IS-0034",
		"Industrial Measuring Tape",
		"Industrial Supplies",
		2800,
		1650,
		15,
		45,
		"Active"
	],
	[
		"PRD-0035",
		"OE-0035",
		"Projector 4500 Lumens",
		"Office Equipment",
		195800,
		149250,
		11,
		42,
		"Active"
	],
	[
		"PRD-0036",
		"F-0036",
		"Office Reception Desk",
		"Furniture",
		170650,
		122850,
		5,
		44,
		"Active"
	],
	[
		"PRD-0037",
		"C-0037",
		"A4 Paper Box",
		"Consumables",
		7650,
		5500,
		37,
		138,
		"Active"
	],
	[
		"PRD-0038",
		"OE-0038",
		"Paper Shredder",
		"Office Equipment",
		32300,
		22650,
		14,
		51,
		"Active"
	],
	[
		"PRD-0039",
		"SE-0039",
		"First Aid Kit Industrial",
		"Safety Equipment",
		13350,
		9100,
		15,
		56,
		"Active"
	],
	[
		"PRD-0040",
		"F-0040",
		"Workstation Desk",
		"Furniture",
		70550,
		49600,
		5,
		33,
		"Active"
	],
	[
		"PRD-0041",
		"OE-0041",
		"Document Scanner",
		"Office Equipment",
		7e4,
		52200,
		15,
		54,
		"Active"
	],
	[
		"PRD-0042",
		"SE-0042",
		"Safety Harness",
		"Safety Equipment",
		16900,
		11500,
		14,
		108,
		"Active"
	],
	[
		"PRD-0043",
		"OE-0043",
		"Label Printer",
		"Office Equipment",
		35950,
		25500,
		14,
		53,
		"Active"
	],
	[
		"PRD-0044",
		"C-0044",
		"Thermal Paper Roll Pack",
		"Consumables",
		3350,
		2e3,
		39,
		192,
		"Active"
	],
	[
		"PRD-0045",
		"E-0045",
		"Circuit Breaker 63A",
		"Electrical",
		4400,
		2800,
		25,
		79,
		"Active"
	],
	[
		"PRD-0046",
		"IN-0046",
		"Enterprise UPS 3KVA",
		"IT & Networking",
		141200,
		107400,
		5,
		43,
		"Active"
	],
	[
		"PRD-0047",
		"IN-0047",
		"Network Rack 24U",
		"IT & Networking",
		67750,
		48250,
		9,
		30,
		"Active"
	],
	[
		"PRD-0048",
		"IN-0048",
		"Network Firewall Appliance",
		"IT & Networking",
		160400,
		117650,
		4,
		46,
		"Active"
	],
	[
		"PRD-0049",
		"SE-0049",
		"Safety Helmet",
		"Safety Equipment",
		2250,
		1400,
		24,
		89,
		"Active"
	],
	[
		"PRD-0050",
		"SE-0050",
		"Industrial Safety Gloves",
		"Safety Equipment",
		1200,
		650,
		23,
		126,
		"Active"
	],
	[
		"PRD-0051",
		"C-0051",
		"Industrial Cleaning Liquid",
		"Consumables",
		7700,
		4850,
		51,
		123,
		"Active"
	],
	[
		"PRD-0052",
		"IS-0052",
		"Hydraulic Jack",
		"Industrial Supplies",
		40400,
		29250,
		8,
		24,
		"Active"
	],
	[
		"PRD-0053",
		"IS-0053",
		"Industrial Trolley",
		"Industrial Supplies",
		50250,
		36300,
		6,
		51,
		"Active"
	],
	[
		"PRD-0054",
		"E-0054",
		"Industrial Socket 32A",
		"Electrical",
		6e3,
		4e3,
		13,
		125,
		"Active"
	],
	[
		"PRD-0055",
		"IN-0055",
		"Network Rack 12U",
		"IT & Networking",
		43600,
		31400,
		12,
		31,
		"Active"
	],
	[
		"PRD-0056",
		"IN-0056",
		"Rack Mount PDU",
		"IT & Networking",
		16900,
		12e3,
		5,
		54,
		"Active"
	],
	[
		"PRD-0057",
		"F-0057",
		"Meeting Room Chair",
		"Furniture",
		25450,
		17350,
		15,
		31,
		"Active"
	],
	[
		"PRD-0058",
		"OE-0058",
		"Thermal Receipt Printer",
		"Office Equipment",
		28050,
		19450,
		14,
		44,
		"Active"
	],
	[
		"PRD-0059",
		"SE-0059",
		"Protective Safety Goggles",
		"Safety Equipment",
		1850,
		1050,
		19,
		73,
		"Active"
	],
	[
		"PRD-0060",
		"F-0060",
		"Visitor Chair",
		"Furniture",
		16450,
		11150,
		7,
		37,
		"Active"
	],
	[
		"PRD-0061",
		"E-0061",
		"Surge Protector",
		"Electrical",
		5600,
		3650,
		29,
		117,
		"Active"
	],
	[
		"PRD-0062",
		"E-0062",
		"Industrial Extension Reel",
		"Electrical",
		13700,
		9e3,
		30,
		124,
		"Active"
	],
	[
		"PRD-0063",
		"C-0063",
		"Hand Sanitizer Carton",
		"Consumables",
		12650,
		8e3,
		60,
		172,
		"Active"
	],
	[
		"PRD-0064",
		"IN-0064",
		"Wi-Fi 6 Access Point",
		"IT & Networking",
		40250,
		28850,
		10,
		49,
		"Active"
	],
	[
		"PRD-0065",
		"SE-0065",
		"Safety Shoes",
		"Safety Equipment",
		9e3,
		5800,
		12,
		95,
		"Active"
	],
	[
		"PRD-0066",
		"SE-0066",
		"Ear Protection Headset",
		"Safety Equipment",
		5300,
		3250,
		32,
		140,
		"Active"
	],
	[
		"PRD-0067",
		"SE-0067",
		"Traffic Safety Cone",
		"Safety Equipment",
		2050,
		1250,
		29,
		100,
		"Active"
	],
	[
		"PRD-0068",
		"IN-0068",
		"8-Port PoE Switch",
		"IT & Networking",
		29e3,
		20900,
		4,
		41,
		"Active"
	],
	[
		"PRD-0069",
		"IN-0069",
		"Business Router",
		"IT & Networking",
		41700,
		30300,
		5,
		55,
		"Active"
	],
	[
		"PRD-0070",
		"E-0070",
		"LED Panel Light 40W",
		"Electrical",
		5100,
		3300,
		32,
		122,
		"Active"
	],
	[
		"PRD-0071",
		"C-0071",
		"Whiteboard Marker Box",
		"Consumables",
		2300,
		1300,
		53,
		122,
		"Active"
	],
	[
		"PRD-0072",
		"E-0072",
		"Electrical Distribution Box",
		"Electrical",
		30050,
		20450,
		18,
		127,
		"Active"
	]
];
var products_default = {
	columns,
	rows
};
//#endregion
export { columns, products_default as default, rows };
